...
*/
void correlate(int rows, int rows_width, const float *data, float *result) {
  // Allocate memory & copy data to GPU
  float* data_GPU = NULL;
  CHECK(cudaMalloc((void**)&data_GPU, rows * rows_width * sizeof(float)));
  float* result_GPU = NULL;
  CHECK(cudaMalloc((void**)&result_GPU, rows * rows_width * sizeof(float)));
  CHECK(cudaMemcpy(data_GPU, data, rows * rows_width * sizeof(float), cudaMemcpyHostToDevice));

  // Run kernel
  dim3 dimBlock(16, 16);
  dim3 dimGrid(divup(rows_width, dimBlock.x), divup(rows, dimBlock.y));
  correlation_kernel<<<dimGrid, dimBlock>>>(result_GPU, data_GPU, rows_width, rows);
  CHECK(cudaGetLastError());

  // Copy data back to CPU & release memory
  CHECK(cudaMemcpy(result, result_GPU, rows * rows_width * sizeof(float), cudaMemcpyDeviceToHost));
